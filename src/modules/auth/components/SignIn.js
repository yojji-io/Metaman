...
export function SignInComponent({
  ...
  return (
    <Form onSubmit={submit}>
      {isElectronMode ? null : <OAuth size={size} onOauth={onOauth} />}
      {isElectronMode ? null : <Divider>{t('general.or')}</Divider>}
      ...
      <Item label={t('auth.password')}>
        {form.getFieldDecorator('password', {
          rules: [{ required: true, message: t('field.is_required') }],
      ...
      <Item>
        ...
        <div className="text-right">
          ...
          <Link to="/sign-up">{t('button.to-signup')}</Link>
        </div>
      </Item>
    </Form>
  );
...
export const SignInForm = compose(Form.create())(SignInComponent);
