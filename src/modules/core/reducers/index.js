...
const history = createReducer({}, {
  ...
  [ACTIONS.REMOVE_HISTORY_ROWS]: (history, { workspace, keys }) => ({
    ...history,
    [workspace]: workspaceHistory({ history }, { workspace, keys })
      .filter((item, i) => !keys.includes(i)),
  }),
});
...
const request = createReducer({ }, {
  ...
  [ACTIONS.REQUEST_DONE]: (state, { response, error }) => ({ loading: false, response, error }),
});

export const settings = createReducer({ proxy: PROXY_TYPES.disabled }, {
  [ACTIONS.SET_SETTINGS_PROXY]: (state, { payload }) => ({ ...state, proxy: payload })
});

export const reducer = ({ history, form, request, firebase, firestore, settings });
